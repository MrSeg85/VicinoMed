...
export default function AdminUsersPlaceholder() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.background }} contentContainerStyle={{ padding: 24 }}>
      <Text style={[styles.title, { color: theme.text }]}>Gestione Utenti</Text>
      <Text style={[styles.sub, { color: theme.textSecondary }]}>Lista, ricerca e moderazione utenti.</Text>
      <View style={[styles.empty, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        ...
        <Text style={[styles.emptyMsg, { color: theme.textSecondary }]}>
          Backend pronto. UI lista utenti con filtri, verifica, sospensione ed eliminazione è il prossimo task.
        </Text>
        <View style={[styles.endpoints, { borderColor: theme.border }]}>
          ...
          <Text style={[styles.endpoint, { color: theme.text }]}>GET /api/admin/users?role=&q=&is_active=&verified=</Text>
          <Text style={[styles.endpoint, { color: theme.text }]}>GET /api/admin/users/{'{id}'}</Text>
          <Text style={[styles.endpoint, { color: theme.text }]}>PATCH /api/admin/users/{'{id}'}/verify</Text>
          <Text style={[styles.endpoint, { color: theme.text }]}>PATCH /api/admin/users/{'{id}'}/suspend</Text>
          <Text style={[styles.endpoint, { color: theme.text }]}>PATCH /api/admin/users/{'{id}'}/activate</Text>
          <Text style={[styles.endpoint, { color: theme.text }]}>DELETE /api/admin/users/{'{id}'}</Text>
        </View>
      </View>
    </ScrollView>
  );
